...
public class MoteFinder
{
	...
	protected final DiscoveryListener listener = new DiscoveryListener()
	{
		...
		public void deviceDiscovered(RemoteDevice device, DeviceClass clazz)
		{
			...
			{
				/***********************************************************************
				 * if (log.isInfoEnabled()) { log.info("found device: " +
				 * device.getFriendlyName(true) + " - " + device.getBluetoothAddress() + " - " +
				 * clazz.getMajorDeviceClass() + ":" + clazz.getMinorDeviceClass() + " - " +
				 * clazz.getServiceClasses()); }/
				 **********************************************************************/

				if (clazz.getMajorDeviceClass() == 1280 && clazz.getMinorDeviceClass() == 4
