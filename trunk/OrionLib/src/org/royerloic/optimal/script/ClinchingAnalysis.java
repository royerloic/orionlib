...
public final class ClinchingAnalysis
{
	...
	private double									mSearchLimit	= 7.0;
	 ...
	 */
	public ClinchingAnalysis(String pGeometryFileName)
	{
		try
		{
			...
			PointPatternP0 lPointPatternP0 = new PointPatternP0();

			boolean lFoundP01 = (mP01 = searchPoint(lSearchTopSheet, "P01", 0, lPointPatternP0)) != null;
			mIndexP01 = (int) (mP01.getCoordinate(0)) - 1;

			PatternExtremumX lPatternPB1 = new PatternExtremumX("maximum", "forwards");

			lSearchTopSheet.positiveDirection();
			boolean lFoundPB1 = (mPB1 = searchPoint(lSearchTopSheet, "PB1", mIndexP01, lPatternPB1)) != null;
			mIndexPB1 = (int) mPB1.getCoordinate(0) - 1;

			PatternExtremumX lPatternPA1 = new PatternExtremumX("minimum", "forwards");

			boolean lFoundPA1 = (mPA1 = searchPoint(lSearchTopSheet, "PA1", mIndexPB1, lPatternPA1)) != null;

			mIndexPA1 = (int) mPA1.getCoordinate(0) - 1;

			lSearchTopSheet.negativeDirection();

			PatternPAp lPatternPAp = new PatternPAp();

			boolean lFoundPAp = (mPAp = searchPoint(lSearchTopSheet, "PAp", mIndexP01, lPatternPAp)) != null;

			mIndexPAp = (int) mPAp.getCoordinate(0) - 1;

			lSearchBottomSheet.negativeDirection();

			boolean lFoundP02 = (mP02 = searchPoint(lSearchBottomSheet, "P02", 0, lPointPatternP0)) != null;

			mIndexP02 = (int) mP02.getCoordinate(0) - 1;

			PatternExtremumX lPatternPB2 = new PatternExtremumX("maximum", "backwards");

			boolean lFoundPB2 = (mPB2 = searchPoint(lSearchBottomSheet, "PB2", mIndexP02, lPatternPB2)) != null;

			mIndexPB2 = (int) mPB2.getCoordinate(0) - 1;

			PatternExtremumX lPatternPA2 = new PatternExtremumX("minimum", "backwards");

			boolean lFoundPA2 = (mPA2 = searchPoint(lSearchBottomSheet, "PA2", mIndexPB2, lPatternPA2)) != null;

			mIndexPA2 = (int) mPA2.getCoordinate(0) - 1;

			boolean lFoundPC2 = (mPC2 = searchPoint(lSearchTopSheet, "PC2", mIndexP01, mPatternPC2)) != null;
																					...
																					final SearchPointList.PointPattern pPattern)
	{
		...
		if (lPointIndex > 0)
		{
			...
			GeometryFile.Point pPoint = pSearchPointList.getPointList().getPointAt(lPointIndex);
	...
	private class PatternExtremumX implements SearchPointList.PointPattern
	{
		...
		public PatternExtremumX(String pMode, String pDirection)
		{
		...
		private boolean conditionOnPoint(GeometryFile.Point pPoint)
		{
			double lCurrentX = pPoint.getCoordinate(1);
			...
			boolean isFound = isExtremum && !mOutOfDomain;
	...
	private class PatternPAp implements SearchPointList.PointPattern
	{
		...
		public boolean conditionOnPointCurrent(GeometryFile.Point pPoint)
		{
			double mDistance = distance(pPoint, mPA1);
	 ...
	 * 
	 */
	private SearchPointList.PointPattern	mPatternPC2	= new SearchPointList.PointPattern()
																										{
	 ...
	 * @return
	 */
	private double distance(GeometryFile.Point pPoint1, GeometryFile.Point pPoint2)
	{
		double dx = pPoint2.getCoordinate(1) - pPoint1.getCoordinate(1);
		double dy = pPoint2.getCoordinate(2) - pPoint1.getCoordinate(2);

		return Math.sqrt(dx * dx + dy * dy);
	};

	private boolean minimum(final double pv1, final double pv2, final double pv3, final double pTol)
	{
		return SearchPointList.isGreaterTol(pv1, pv2, pTol) && SearchPointList.isGreaterTol(pv3, pv2, pTol);
	}

	private boolean maximum(final double pv1, final double pv2, final double pv3, final double pTol)
	{
		return SearchPointList.isGreaterTol(pv2, pv1, pTol) && SearchPointList.isGreaterTol(pv2, pv3, pTol);
	}
