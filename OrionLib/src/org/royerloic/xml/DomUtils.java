...
public final class DomUtils
{
	...
	{
		List<Node> lSubNodeList = new ArrayList<Node>();
	...
	{
		NamedNodeMap lMap = pNode.getAttributes();
		String lResult = null;
		if (lMap != null)
		{
			int len = lMap.getLength();
			Attr lAttr;
			for (int i = 0; i < len; i++)
			{
	...
	{
		Node lChildNode = (Element) pDoc.createElement(pName);
		return lChildNode;
	}
	...
	{
		File lXmlFile = new File(pFileName);
		...
		try
		{
			...
			try
			{
				DOMSource lDOMSource = new DOMSource(pDoc);
				lFileOutputStream = new FileOutputStream(lXmlFile);
				StreamResult lStreamResult = new StreamResult(lFileOutputStream);
