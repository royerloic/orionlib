...
public class misc
{
	...
	/**
	 * Value of the beta function evaluated with arguments a and b.
	 * 
	 * This routine is a translation into C of a Fortran subroutine by W.
	 * Fullerton of Los Alamos Scientific Laboratory. Some modifications have been
	...
	/**
	 * Determine the number of terms for the double precision orthogonal Chebyshev
	 * series "dos" needed to insure the error is no larger than "eta". Ordinarily
	 * eta will be chosen to be one-tenth machine precision.
	 * 
	 * These routines are translations into C of Fortran routines by W. Fullerton
	 ...
	 * SYNOPSIS
	 * 
	 * #include "DistLib.h" double choose(double n, double k); double
	 * fastchoose(double n, double k); double lchoose(double n, double k); double
	...
	public static double d1mach(int i)
	{
		...
		case 3:
			return java.lang.Math.pow(i1mach(10), -(double) i1mach(14));
		case 4:
			return java.lang.Math.pow(i1mach(10), 1 - (double) i1mach(14));
		case 5:
	 ...
	 * SYNOPSIS
	 * 
	 * #include "DistLib.h" double fmod(double x, double y);
	 * 
	 * DESCRIPTION
	 * 
	 ...
	 * NOTES
	 * 
	 * It may be better to use the system version of this function, but this
	 * version is portable.
	 */
	 ...
	 * made so that the routines conform to the IEEE 754 standard.
	 * 
	 * Improvements by Martin Maechler, May 1997 Note that the code could be
	 * further improved by using java.lang.Math.pow(x, i) instead of pow(x,
	 * (double)i)
	 */
	 ...
	 * SYNOPSIS
	 * 
	 * #include "DistLib.h" double fround(double x, double digits);
	 * 
	 * DESCRIPTION
	 * 
	 ...
	 * SYNOPSIS
	 * 
	 * #include "DistLib.h" double fsign(double x, double y);
	 * 
	 * DESCRIPTION
	 * 
	 ...
	 * SYNOPSIS
	 * 
	 * #include "DistLib.h" double fsquare(double x);
	 * 
	 * DESCRIPTION
	 * 
	 ...
	 * SYNOPSIS
	 * 
	 * #include "DistLib.h" double gammafn(double x);
	 * 
	 * DESCRIPTION
	 * 
	 ...
	 * NOTES
	 * 
	 * This function is a translation into C of a Fortran subroutine by W.
	 * Fullerton of Los Alamos Scientific Laboratory.
	 * 
	 * The accuracy of this routine compares (very) favourably with those of the
	...
	public static double gamma_cody(double x)
	{
		 ...
		 * Computation is based on an algorithm outlined in reference [1]. The
		 * program uses rational functions that approximate the GAMMA function to at
		 * least 20 significant decimal digits. Coefficients for the approximation
		 * over the interval (1,2) are unpublished. Those for the approximation for
		 * X >= 12 are from reference [2]. The accuracy achieved depends on the
		 ...
		 * 
		 * 
		 * References: [1] "An Overview of Software Development for Special
		 * Functions", W. J. Cody, Lecture Notes in Mathematics, 506, Numerical
		 ...
		 * 
		 * Authors: W. J. Cody and L. Stoltz Applied Mathematics Division Argonne
		 ...
		 * 
		 * beta - radix for the floating-point representation maxexp - the smallest
		 * positive power of beta that overflows XBIG - the largest argument for
		 * which GAMMA(X) is representable in the machine, i.e., the solution to the
		 * equation GAMMA(XBIG) = beta**maxexp XINF - the largest machine
		 * representable floating-point number; approximately beta**maxexp EPS - the
		 * smallest positive floating-point number such that 1.0+EPS > 1.0 XMININ -
		 ...
		 * beta maxexp XBIG
		 * 
		 * CRAY-1 (S.P.) 2 8191 966.961 Cyber 180/855 under NOS (S.P.) 2 1070
		 * 177.803 IEEE (IBM/XT, SUN, etc.) (S.P.) 2 128 35.040 IEEE (IBM/XT, SUN,
		 * etc.) (D.P.) 2 1024 171.624 IBM 3033 (D.P.) 16 63 57.574 VAX D-Format
		 * (D.P.) 2 127 34.844 VAX G-Format (D.P.) 2 1023 171.489
		 * 
		 * XINF EPS XMININ
		 * 
		 * CRAY-1 (S.P.) 5.45E+2465 7.11E-15 1.84E-2466 Cyber 180/855 under NOS
		 * (S.P.) 1.26E+322 3.55E-15 3.14E-294 IEEE (IBM/XT, SUN, etc.) (S.P.)
		 * 3.40E+38 1.19E-7 1.18E-38 IEEE (IBM/XT, SUN, etc.) (D.P.) 1.79D+308
		 * 2.22D-16 2.23D-308 IBM 3033 (D.P.) 7.23D+75 2.22D-16 1.39D-76 VAX
		 * D-Format (D.P.) 1.70D+38 1.39D-17 5.88D-39 VAX G-Format (D.P.) 8.98D+307
		 * 1.11D-16 1.12D-308
		 * 
		 * ******************************************************************
		 * 
		...
		L_end:
		{
			...
			else if (y < 12.)
			{
				...
				else
				{
					/*
					 * ----------------------------------------------------------- 1 <=
					 * argument < 12, reduce argument if necessary
					 * -----------------------------------------------------------
					 */
					n = (long) y - 1;
					y -= n;
				...
				/*
				 * --------------------------------------------------------- Evaluate
				 * approximation for 1.0 < argument < 2.0
				 * ---------------------------------------------------------
				 */
				xnum = 0.;
				xden = 1.;
				...
				if (y1 < y)
				{
					/*
					 * -------------------------------------------------------- Adjust
					 * result for case 0.0 < argument < 1.0
					 * --------------------------------------------------------
					 */
					res /= y1;
				}
				else if (y1 > y)
				{
					/*
					 * ---------------------------------------------------------- Adjust
					 * result for case 2.0 < argument < 12.0
					 * ----------------------------------------------------------
					 */
					for (i = 0; i < n; ++i)
					{
	 ...
	 * DESCRIPTION
	 * 
	 * This function alculates the minimum and maximum legal bounds for x in
	...
	public static int i1mach(int i)
	{
		...
		case 10:
			return Constants.FLT_RADIX;

		case 11:
			return Constants.FLT_MANT_DIG;
		case 12:
			return Constants.FLT_MIN_EXP;
		case 13:
			return Constants.FLT_MAX_EXP;

		case 14:
			return Constants.DBL_MANT_DIG;
		case 15:
			return Constants.DBL_MAX_EXP;
		case 16:
	 ...
	 * SYNOPSIS
	 * 
	 * #include "DistLib.h" int imax2(int x, int y);
	 * 
	 * DESCRIPTION
	 * 
	 ...
	 * SYNOPSIS
	 * 
	 * #include "DistLib.h" int Math.min(int x, int y);
	 * 
	 * DESCRIPTION
	 * 
	 ...
	 * SYNOPSIS
	 * 
	 * #include "DistLib.h" double lbeta(double a, double b);
	 * 
	 * DESCRIPTION
	 * 
	 ...
	 * SYNOPSIS
	 * 
	 * #include "DistLib.h" extern int signgam; double lgammafn(double x);
	 * 
	 * DESCRIPTION
	 * 
	 * This function computes log|gamma(x)|. At the same time the variable
	 * "signgam" is set to the sign of the gamma function.
	 * 
	 * NOTES
	 * 
	 * This routine is a translation into C of a Fortran subroutine by W.
	...
	public static double lgammafn(double x)
	{
		...
		{ /* Negative integer argument */
			throw new java.lang.ArithmeticException("Math Error: RANGE");
			// return Double.POSITIVE_INFINITY;/* +Inf, since lgamma(x) =
			// log|gamma(x)| */
		}
	 ...
	 * SYNOPSIS
	 * 
	 * #include "DistLib.h" double lgammacor(double x);
	 * 
	 * DESCRIPTION
	 * 
	 ...
	 * NOTES
	 * 
	 * This routine is a translation into C of a Fortran subroutine written by W.
	 * Fullerton of Los Alamos Scientific Laboratory.
	 */
	 ...
	 * SYNOPSIS
	 * 
	 * #include "DistLib.h" double dlnrel(double x);
	 * 
	 * DESCRIPTION
	 * 
	 ...
	 * NOTES
	 * 
	 * This code is a translation of a Fortran subroutine of the same name written
	 ...
	 * 
	 * case "Math Error: NONE": (!!!!fixme!!!!) = 0; break;
	 * 
	 * case "Math Error: DOMAIN": case "Math Error: NOCONV": (!!!!fixme!!!!) =
	 * EDOM; break;
	 * 
	 ...
	 * SYNOPSIS
	 * 
	 * #include "DistLib.h" double sign(double x);
	 * 
	 * DESCRIPTION
	 * 
