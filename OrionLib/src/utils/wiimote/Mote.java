...
import utils.wiimote.event.DataEvent;
import utils.wiimote.event.DataListener;
...
import utils.wiimote.request.CalibrationDataRequest;
import utils.wiimote.request.PlayerLedRequest;
import utils.wiimote.request.RawByteRequest;
...
public class Mote
{
	...
	{
		// 1. Disable IR Camera
		outgoing.sendRequest(new RawByteRequest(new byte[]
		{ 82, 19, 0 }));
	...
	{
		if (calibrationDataReport == null && error == 0
				&& address[0] == 0x00
				&& address[1] == 0x20)
		{
			// calibration data (most probably), if thats the first time, it's
			// probably for us - so we'll consume this event.
			CalibrationDataReport report = new CalibrationDataReport(	payload[0],
																																payload[1],
																																payload[2],
																																payload[4],
																																payload[5],
