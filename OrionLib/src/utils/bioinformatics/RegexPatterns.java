...
public class RegexPatterns
{
	...
	{
		return StringUtils.matches(pCharSequence, sAbbreviationPatternType2)
				&& ((StringUtils.countUpperCase(pCharSequence) + StringUtils.countDigits(pCharSequence)) > pCharSequence
						.length() / 4);
	}
	...
	public static boolean isAbbreviationType3(final CharSequence pCharSequence)
	{
		...
		for (final String lToken : lTokenArray)
			isAbbreviationType3 &= isAbbreviationType1(lToken) || isAbbreviationType2(lToken) || isNumber(lToken);
	...
	public static boolean isGarbage(final CharSequence pCharSequence)
	{
		if (StringUtils.submatches(pCharSequence, "(H|h)ypothetical"))
			return true;
		else if (StringUtils.submatches(pCharSequence, "(C|c)hromosome.*(O|o)pen\\W+(R|r)eading\\W+(F|f)rame"))
			return true;
